...
async def main():
    ...
    logger.info(f"Starting data generation process for category: {args.category}")
    ...
    try:
        ...
        generator = DataGenerator(bedrock_client, args.category)
        logger.info(f"Initialized data generator for category: {args.category}")
        ...
        logger.info(f"  - all_tier3_{args.category}.csv (aggregated Tier 3 data)")
        logger.info(f"  - tier3_{args.category}_[practice].csv files (separate file for each Tier 2 practice)")
